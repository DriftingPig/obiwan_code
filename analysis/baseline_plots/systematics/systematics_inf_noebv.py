...
obiwan = fits.getdata(topdir_subset + "subset_more_rs0.fits")
...
coeffs = {'g':0.00, 'r':0.00, 'z':0.00, 'w1':0.00}
...
for band in ['g','r','z']:
    ...
    for i in range(0,8):
        ...
        ratio = sel_lrg.sum()/ssel.sum()/ave
    ...
    for l in range(10):
        if y_error is not None:
            y_error += (np.array(y_jk[0])-np.array(y))**2
        else:
            y_error = (np.array(y_jk[0])-np.array(y))**2
...
np.savetxt(savedir+'obiwan_g_ebv0_more_rs0.txt', obiwan_psf_g)
np.savetxt(savedir+'obiwan_r_ebv0_more_rs0.txt', obiwan_psf_r)
np.savetxt(savedir+'obiwan_z_ebv0_more_rs0.txt', obiwan_psf_z)
