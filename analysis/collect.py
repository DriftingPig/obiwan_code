...
class Collect(LegacySimData):
    ...
    def compute_photoz(self, totslice = None, sliceidx = None):
            topdir = os.path.dirname(os.path.abspath(self.outdir))+'/subset'
            if totslice==1:
                output_fn = topdir+'/subset_%s.fits'%(self.subsection)
            else:
            ...
            print("running photo-z")
            photoz_main(output_fn, 'input', self.region)
            photoz_main(output_fn, 'output', self.region)
