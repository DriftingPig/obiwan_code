...
def stage_fit_on_coadds(
    ...
    N_band = {"g":0,"r":0,"z":0}
    N_nano2e = {"g":0,"r":0,"z":0}
    for tim in tims:
        N_band[tim.band]+=1
        N_nano2e[tim.band]+=tim.zpscale*tim.gain
        ie = tim.inverr
        if np.any(ie < 0):
            print('Negative inverse error in image {}'.format(tim.name))
    for band in ["g","r","z"]:
        if N_band[band]>0:
            N_nano2e[band] = N_nano2e[band]/N_band[band]
    ...
    for band,img,iv,allmask,anymask,psfimg in zip(
        ...
        fitoncoadds_reweight_ivar = False
