...
class LegacySersicIndexSim(SersicIndex):
    def __init__(self, val=0):
        super(LegacySersicIndexSim, self).__init__(val=val)
        self.lower = 0.2
        self.upper = 7.0
        self.maxstep = 0.25
...
class LegacySurveyData(object):
    ...
    def find_file(self, filetype, brick=None, brickpre=None, band='%(band)s',
        ...
        *output*: True if we are about to write this file; will use self.outdir as
        ...
        elif filetype == 'galaxy-sims-elg':
            return swap(os.path.join(basedir, 'tractor', brickpre,
                                     'galaxy-sims-elg-%s.fits' % brick))
    ...
    def get_ccds(self, **kwargs):
        ...
        try:
            import os
            cosmos_num = os.environ['cosmos_section']
            brickname = os.environ['BRICKNAME']
            ccd_fn = '/global/cscratch1/sd/dstn/dr9-cosmos-subs/%s/coadd/%s/%s/legacysurvey-%s-ccds.fits'%(cosmos_num,brickname[:3],brickname,brickname)
            fns=[ccd_fn]
        except:
            pass
