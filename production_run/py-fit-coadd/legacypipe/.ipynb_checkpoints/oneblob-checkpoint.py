...
def one_blob(X):
    ...
    (nblob, iblob, Isrcs, brickwcs, bx0, by0, blobw, blobh, blobmask, timargs,
     ...
     large_galaxies_force_pointsource, less_masking, frozen_galaxies, force_model) = X
    ...
    ob = OneBlob('%i'%(nblob+1), blobwcs, blobmask, timargs, srcs, bands,
                 ...
                 less_masking, frozen_galaxies, force_model = force_model)
...
class OneBlob(object):
    def __init__(self, name, blobwcs, blobmask, timargs, srcs, bands,
                 plots, ps, use_ceres, refmap,
                 large_galaxies_force_pointsource,
                 less_masking, frozen_galaxies, force_model):
        self.name = name
    ...
    def run(self, B, reoptimize=False, iterative_detection=True,
        ...
        B = self.run_model_selection(cat, Ibright, B,
                                     iterative_detection=iterative_detection, force_model = self.force_model)
    ...
    def run_model_selection(self, cat, Ibright, B, iterative_detection=True, force_model = None):
        ...
        for numi,srci in enumerate(Ibright):
            ...
            keepsrc = self.model_selection_one_source(src, srci, models, B, force_model = force_model)
    ...
    def model_selection_one_source(self, src, srci, models, B, force_model=None):
            ...
            if name == 'ser' and newsrc is None:
                ...
                if smod not in ['dev', 'exp']:
                    if force_model is None:
                        ...
                        #src is psf/exp, more similar to exp
        ...
        if force_model is not None:
            print("original model is %s"%keepmod)
            keepmod = force_model
            print("force model to be %s"%force_model)
