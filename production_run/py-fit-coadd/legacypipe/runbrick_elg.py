...
def stage_srcs(pixscale=None, targetwcs=None,
    ...
    if mode == "to_fits":
        ...
        outdir = os.path.dirname(os.path.dirname(random_fn))
        fn = os.path.join(outdir,"output","rs0","coadd",brickname[:3],brickname,"brick_%s_img.pickle"%(brickname))
        subprocess.call(["rm",fn])
        #T = fitsio.FITS(fn,'rw')
        pickle.dump(tims, open( fn, "wb" ))
        #tim.toFits(T)
        #survey.outdir
        #output/rs0/coadd/148/1486p002/
        #fn = os.path.join([outdir,"output","rs0","coadd","%s","%s","brick_%s_img_%s.fits"%(brickname[:3], brickname, brickname, tim.band)])
        #T.writeto(fn)
        print("written %s"%fn)
        return None
    ...
    if mode == "from_fits":
        ...
        for band in ["g","r","z"]:
            random_fn = os.environ["RANDOMS_FROM_FITS"]
            outdir = os.path.dirname(os.path.dirname(random_fn))
            fn = os.path.join(outdir,"output","rs0","coadd",brickname[:3],brickname,"brick_%s_img_%s.fits"%(brickname, band))
            T = fitsio.FITS(fn,'r')
    ...
    simcat_new.current_gflux = -np.ones_like(simcat_new.ra)
    simcat_new.current_rflux = -np.ones_like(simcat_new.ra)
    simcat_new.current_zflux = -np.ones_like(simcat_new.ra)
    #the goal
    ...
    simcat_new.goal_gflux = np.zeros_like(simcat_new.gflux)
    simcat_new.goal_rflux = np.zeros_like(simcat_new.rflux)
    simcat_new.goal_zflux = np.zeros_like(simcat_new.zflux)
    ...
    simcat_new.origin_gflux = survey.simcat.gflux.copy()
    simcat_new.origin_rflux = survey.simcat.rflux.copy()
    simcat_new.origin_zflux = survey.simcat.zflux.copy()
    ...
    while i<N_tot:
        ...
        while N_fail<3 and maxiter<60:
            ...
            if chi2_old is None:
                ...
                gflux = BB.sources[idx].brightness.g.copy()
                rflux = BB.sources[idx].brightness.r.copy()
                zflux = BB.sources[idx].brightness.z.copy()
                simcat_new.origin_gflux[i] = gflux
                simcat_new.origin_rflux[i] = rflux
                simcat_new.origin_zflux[i] = zflux
            ...
            if chi2_new<1e-4:
                if chi2_new<chi2_old:
                    ...
                    e2_current = e2 
                    rhalf_current = rhalf 
                    gflux_current = BB.sources[idx].brightness.g.copy()
                    rflux_current = BB.sources[idx].brightness.r.copy()
                    zflux_current = BB.sources[idx].brightness.z.copy()
                break
            else:
                if chi2_new<=chi2_old:
                    N_fail=0
        ...
        simcat_new.current_gflux[i] = gflux_current
        simcat_new.current_rflux[i] = rflux_current
        simcat_new.current_zflux[i] = zflux_current
        ...
        simcat_new.goal_gflux[i] =  obj_origin.gflux
        simcat_new.goal_rflux[i] =  obj_origin.rflux
        simcat_new.goal_zflux[i] =  obj_origin.zflux
        if re_fit is False:
