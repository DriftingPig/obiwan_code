...
def reprocess_wcat(wcat, brickname,blobmap,maskbits,T,survey):
	...
	if dataset == 'normal':
			...
			if survey.region == 'decam':
				region = 'south'
			else:
				region = 'north'
			fn_dr9_tractor = '/global/cfs/cdirs/cosmo/data/legacysurvey/dr9/'+region+'/tractor-i/'+brickname[:3]+'/tractor-i-'+brickname+'.fits'
	...
	for i in range(len(add_source)):
		...
		brightness =  tractor.NanoMaggies(g=1.,r=1.,z=1., order=['g','r','z'])
...
class BuildStamp_wise():
    ...
    def galaxy(self, obj,bandname,fluxfactor,band):
        ...
        n,ra,dec,r_half,e1,e2,flux, mw_transmission = float(obj.n),float(obj.ra),float(obj.dec),float(obj.rhalf),float(obj.e1),float(obj.e2), float(obj.get('w%sflux'%band)), float(obj.get('mw_transmission_w%s'%band))
...
def noise_for_galaxy(gal,nano2e,nims):
    ...
    # rescale
    nims_down = nims.copy()
    nims_down.fill(np.clip(nims_down.array,1,nims.array.max()))
