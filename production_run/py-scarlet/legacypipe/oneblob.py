...
def one_blob(X):
    ...
    B = ob.run_scarlet(B, reoptimize=reoptimize, iterative_detection=iterative)
...
class OneBlob(object):
    ...
    def run_scarlet(self, B, reoptimize=False, iterative_detection=None,
            compute_metrics=None):
        import scarlet
        images = np.array([self.tims[0].data, self.tims[1].data, self.tims[2].data])
        filters = np.array(['g','r','z'])
        new_srcs = []
        for src in self.srcs:
            if src is not None:
                new_srcs.append(src)
        self.srcs = new_srcs 
        _,x0,y0 = self.blobwcs.radec2pixelxy(np.array([src.getPosition().ra  for src in self.srcs]),np.array([src.getPosition().dec for src in self.srcs]))
        centers = [(src[0]-1, src[1]-1) for src in np.array([y0,x0]).transpose()]
        #import pdb;pdb.set_trace()
        weights = np.array([self.tims[0].invvar, self.tims[1].invvar, self.tims[2].invvar])
        psfi = np.array([self.tims[0].psf.getImage(0,0), self.tims[1].psf.getImage(0,0), self.tims[2].psf.getImage(0,0)])
        psf = scarlet.ImagePSF(psfi)
        psfi = np.array([self.tims[0].psf.getImage(0,0).copy(), self.tims[1].psf.getImage(0,0).copy(), self.tims[2].psf.getImage(0,0).copy()])
        model_psf = scarlet.ImagePSF(psfi) 
        #scarlet.GaussianPSF(sigma=(1.0,)*len(filters)): *** TypeError: '>' not supported between instances of 'tuple' and 'int'
        ...
        if len(skipped)>0:
            #in this case, just assume previous fitting is true
            new_sources = []
            print(skipped)
            print("skipped in scarlet")
            for i in len(self.srcs):
                if i in skipped:
                    if self.srcs[i].getSourceType()=='PointSource':
                        new_src = scarlet.PointSource(model_frame, centers[i], observation)
                    else:
                        new_src = scarlet.ExtendedSource(model_frame, center, observation, K=1)
        ...
        for k, src in enumerate(sources):
            all_flux = scarlet.measure.flux(src)
            self.srcs[k].brightness.g = all_flux[0]
            self.srcs[k].brightness.r = all_flux[1]
            self.srcs[k].brightness.z = all_flux[2]
        plots = True
        if plots:
            import matplotlib
    ...
    def run(self, B, reoptimize=False, iterative_detection=True,
        ...
        #import pdb;pdb.set_trace()
        #if iterative_detection is False:
        #    self.plots = True
        trun = tlast = Time()
