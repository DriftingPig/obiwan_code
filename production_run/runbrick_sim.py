...
class SimDecals(LegacySurveyData):
    def __init__(self, dataset=None, survey_dir=None, metacat=None, simcat=None,
                 output_dir=None,add_sim_noise=False, seed=0, brickname=None,region=None,
                 **kwargs):
    ...
    def get_ccds(self, **kwargs):
        if self.dataset == 'cosmos':
            ...
            #deep hack: read ccds from dr9.1.1
            import os
            brickname = os.environ["BRICKNAME"]
            ccd_fn = "/global/project/projectdirs/cosmo/work/legacysurvey/dr9.1.1/coadd/%s/%s/legacysurvey-%s-ccds.fits"%(brickname[:3],brickname,brickname)
            fns=[ccd_fn]
            TT = []
            for fn in fns:
                debug('Reading CCDs from', fn)
            ...
            import os
            if self.region == 'decam':
                brick_region = 'south'
            else:
                brick_region = 'north'
            brickname = os.environ["BRICKNAME"]
            ccd_fn = "/global/project/projectdirs/cosmo/data/legacysurvey/dr9/%s/coadd/%s/%s/legacysurvey-%s-ccds.fits"%(brick_region,brickname[:3],brickname,brickname)
            T = fits_table(ccd_fn, **kwargs)
            debug('Got', len(T), 'CCDs')
            T = self.cleanup_ccds_table(T)
            return T
...
def noise_for_galaxy(gal,nano2e,rng):
    ...
    num_stds= rng.randn(one_std_per_pix.shape[0],one_std_per_pix.shape[1])
...
def get_srcimg_invvar(stamp_ivar,img_ivar):
    ...
    img_ivar_array = np.clip(img_ivar.array,1e-20, img_ivar.array.max())
    ivar= np.power(stamp_ivar.array.copy(), -1) + np.power(img_ivar_array, -1)
...
class SimImage(object):
    def __init__(self, survey, t):
        ...
        self.sim_fit_coadd = survey.sim_fit_coadd
    ...
    def get_tractor_image(self, **kwargs):
        ...
        if tim is None: # this can be None when the edge of a CCD overlaps
            return tim
        if self.sim_fit_coadd:
            print("sim fit coadd: skipping adding sim during stage tim")
            return tim
        ...
        #trying to make runs stable by make random seed the same across runs
        semi_seed = int(1e5*tim.data[0][0])%2**30
        rng = np.random.RandomState(self.brickid+semi_seed)
        for ii, obj in enumerate(self.survey.simcat):
            ...
            if self.survey.add_sim_noise:
                stamp += noise_for_galaxy(stamp,objstamp.nano2e,rng)
...
def get_parser_sim():
    ...
    parser.add_argument('--dataset', default='normal', help='choose from normal, cosmos, deep')
    ...
    parser.add_argument('--sim_fit_coadd', action='store_true', default=False, help="sim fit on coadd, avoid adding sim at tim stage")
    parser.add_argument('--no_write_pickle',action='store_true',default=False,help='do not write pickles')
    return parser
...
def build_simcat(Samp=None, brickname = None, radius_in_deg=0./3600):
...
def rm_checkpoints(survey,brickname):
    import os
    import subprocess
    checkpoints_fn = os.path.join(survey.output_dir,'checkpoints',brickname[:3],'checkpoint-%s.pickle'%brickname)
    subprocess.call(["rm","-rf",checkpoints_fn])
    return None
...
def main():
    ...
    sim_fit_coadd = optdict.pop('sim_fit_coadd')
    no_write_pickle = optdict.pop('no_write_pickle')

    N_tot = len(fits_table(random_fn))
    new_nobj = np.clip(0,N_tot-startid,nobj)
    if new_nobj<nobj:
        print("not enough targets to add, adding %d targets instead"%new_nobj)
        nobj = new_nobj
    ...
    import logging
    if len(simcat)==0:
        print("Obiwan: simcat length is 0, return None")
        return None
    ...
    survey = SimDecals(survey_dir=opt.survey_dir, subset=subset, simcat = simcat, dataset=dataset,
                          output_dir=opt.output_dir,brickname=brickname, add_sim_noise=add_sim_noise, region = opt.run)
    survey.sim_fit_coadd = sim_fit_coadd
    print('Using survey:', survey)
    #print('with output', survey.output_dir)

    print('opt:', opt)
    if no_write_pickle:
        kwargs.update(write_pickles=False)
    else:
        kwargs.update(write_pickles=True)
    print('kwargs:', kwargs)

    run_brick(opt.brick, survey, **kwargs)
    rm_checkpoints(survey,brickname)
    return 0
